...
// ── Shared styles ────────────────────────────────────────────────────────────
const inp = "w-full bg-white dark:bg-[#161B22] border border-gray-200 dark:border-gray-800 rounded-xl py-2 px-4 text-sm font-bold h-[42px] focus:outline-none focus:border-blue-500 focus:ring-4 ring-blue-500/5 transition-all text-gray-900 dark:text-gray-100 placeholder:text-gray-400 shadow-sm hover:border-gray-300 dark:hover:border-gray-700";
const inpSm = "w-full bg-white dark:bg-[#0D1117] border border-gray-200 dark:border-gray-800 rounded-lg py-1.5 px-3 text-sm font-bold h-[36px] focus:outline-none focus:border-blue-400 transition-all text-gray-800 dark:text-gray-200";
      ...
      <button onClick={() => setOpen(!open)} className={`w-full flex items-center justify-between px-6 py-5 ${t.bg} hover:opacity-95 transition-all text-left select-none group border-b ${t.border}`}>
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            {badge && <span className={`mt-1 inline-block px-2.5 py-1 rounded-full text-xs font-bold tracking-normal border transition-colors ${t.badge}`}>{badge}</span>}
          </div>
        </div>
    ...
    <div className={`p-5 rounded-3xl border border-l-8 ${config} transition-all duration-300 shadow-sm hover:shadow-md animate-in fade-in slide-in-from-bottom-2`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <p className={`text-sm font-bold tracking-normal ${textColor}`}>{title}</p>
        </div>
        <span className="text-xs bg-white dark:bg-black/40 text-gray-700 dark:text-gray-200 px-3 py-1 rounded-full font-bold border border-gray-100 dark:border-gray-800 shadow-inner transition-colors">{items.length} Items</span>
      </div>
      ...
      <div className="space-y-3">
        ...
        <button onClick={() => onUpdate([...items, { key: '', value: '' }])}
          className={`w-full py-2.5 border-2 border-dashed rounded-[1.2rem] text-xs font-bold tracking-normal transition-all flex items-center justify-center gap-2 mt-2 
          ...
          <Plus className="w-3.5 h-3.5" /> Add {title.split(' ')[0]}
        </button>
      </div>
    </div>
  );
    ...
    <div className="animate-in fade-in duration-700 flex flex-col pb-20 pt-4">
      ...
      <div className="flex items-center justify-between mb-8 px-2">
        <div>
          ...
          <p className="text-xs font-bold text-gray-400 mt-1 ml-4 tracking-widest leading-none opacity-60">Config & Layer Orchestration</p>
        </div>
        
        <div className="flex items-center gap-3">
          <button 
            onClick={() => setShowFlow(true)}
            className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-indigo-500/10 text-indigo-500 hover:bg-indigo-500 hover:text-white text-xs font-black transition-all tracking-wider border border-indigo-500/20 group shadow-sm active:scale-95 ring-4 ring-indigo-500/0 hover:ring-indigo-500/10"
          >
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="relative group">
            <p className="text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 flex items-center gap-1.5 group-hover:text-blue-500 transition-all">
            ...
            {activeTooltip === 'appName' && <div className="absolute left-0 -top-12 bg-gray-900 text-white border border-gray-700 p-2.5 text-xs rounded-xl shadow-2xl w-[220px] z-20">{t.dockerfile.appNameTooltip}</div>}
            <input type="text" value={appName} onChange={(e) => setAppName(e.target.value)} className={inp} placeholder="my-awesome-app" />
            <div className="text-xs text-gray-400 mt-2 font-bold italic opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
          ...
          <div className="relative group">
            ...
            {activeTooltip === 'baseImage' && <div className="absolute right-0 -top-12 bg-gray-900 text-white border border-gray-700 p-2.5 text-xs rounded-xl shadow-2xl w-[220px] z-20">{t.dockerfile.baseImageTooltip}</div>}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-blue-600 dark:text-blue-400 tracking-normal leading-none">{t.dockerfile.startConfig}</p>
                  <p className="text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.executionLogic}</p>
                </div>
              </div>

              <div className="flex items-center gap-6">
                <Checkbox checked={useShellWrapper} onChange={(val: boolean) => setUseShellWrapper(val)} label="Use SH -EC Wrapper" theme="yellow" />
              </div>
            </div>
            ...
            {(entrypoint || startCmd) && (
              <div className="px-5 py-2 bg-white/80 dark:bg-black/40 rounded-2xl border border-blue-100 dark:border-blue-900/30 shadow-sm backdrop-blur-md transition-transform hover:scale-[1.02]">
                <p className="text-xs font-mono text-blue-600 dark:text-blue-400 truncate max-w-[300px]">
                  <span className="opacity-50 text-xs font-sans mr-2 font-bold">Preview:</span>
                  {useShellWrapper ? 'sh -ec "' : ''}<span className="font-bold">{entrypoint || '<ENTRYPOINT>'}</span> {startCmd || '<CMD>'}{useShellWrapper ? '"' : ''}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative items-start">
            ...
            <div className="space-y-3 relative group">
              <div className="flex items-center justify-between px-1">
                ...
                <span className="text-xs font-bold text-blue-700 bg-blue-100 dark:text-blue-300 dark:bg-blue-800/60 px-3 py-1 rounded-full shadow-sm">FIXED</span>
              </div>
            ...
            <div className="space-y-3 relative group">
              <div className="flex items-center justify-between px-1">
                ...
                <span className="text-xs font-bold text-emerald-700 bg-emerald-100 dark:text-emerald-300 dark:bg-emerald-800/60 px-3 py-1 rounded-full shadow-sm">ARGS</span>
              </div>
              ...
              <p className="text-xs text-emerald-600 dark:text-emerald-400 px-2 leading-relaxed font-bold italic">{t.dockerfile.cmdDesc}</p>
            </div>
          </div>
        </div>
      </Section>
          ...
          <div className="p-8 bg-gray-50/50 dark:bg-[#161B22]/50 border border-gray-200 dark:border-gray-800 rounded-[2.5rem] shadow-inner relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-800 dark:text-gray-200 tracking-normal">{t.dockerfile.run}</p>
                  <p className="text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.layerScripts}</p>
                </div>
              </div>
              <button onClick={() => setRunCmds([...runCmds, ""])} className={`${btnSm} !bg-white dark:!bg-[#0D1117] border-orange-200 text-orange-600 hover:bg-orange-50 hover:border-orange-500 transition-all duration-300 shadow-md active:translate-y-0.5`}><Plus className="w-4 h-4" /> {t.dockerfile.addStep}</button>
            </div>
            ...
            <div className="space-y-4 relative z-10">
              {runCmds.map((cmd, i) => (
                <div key={i} className="group/row relative animate-in slide-in-from-left-2 duration-500">
                  ...
                  <textarea
                    ...
                    className="w-full bg-[#1A1A1A] text-orange-100 p-5 pl-8 text-sm font-mono leading-relaxed rounded-2xl focus:outline-none focus:ring-4 ring-orange-500/10 transition-all border border-transparent focus:border-orange-500/30 selection:bg-orange-500/30 shadow-xl"
                  />
              ...
              {runCmds.length === 0 && <div className="py-10 border-2 border-dashed border-orange-200 dark:border-orange-800 rounded-[2rem] flex flex-col items-center justify-center opacity-40 italic text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal">{t.dockerfile.noSteps}</div>}
            </div>
          </div>
          ...
          <div className="pt-10 border-t-2 border-gray-100 dark:border-gray-800">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-800 dark:text-gray-200 tracking-normal">{t.dockerfile.addCopy}</p>
                  <p className="text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.srcDestMapping}</p>
                </div>
              </div>
              <button onClick={() => setCopyAddItems([...copyAddItems, { type: "COPY", from: "", chown: "", src: "", dest: "" }])} className={`${btnSm} !bg-white dark:!bg-[#0D1117] border-blue-200 text-blue-600 hover:bg-blue-50 hover:border-blue-500 transition-all duration-300 shadow-md active:translate-y-0.5`}><Plus className="w-4 h-4" /> {t.dockerfile.addRule}</button>
            </div>
            <div className="grid grid-cols-1 gap-4">
                ...
                <div key={i} className="flex flex-col lg:flex-row gap-4 items-center bg-gray-50/50 dark:bg-[#161B22]/50 p-4 rounded-3xl border border-gray-100 dark:border-gray-800 hover:border-blue-400/50 transition-all group/file shadow-sm">
                  <div className="flex items-center gap-3">
                    <select value={item.type} onChange={(e) => updateArrayField(copyAddItems, setCopyAddItems, i, "type", e.target.value)}
                      className="bg-white dark:bg-[#0D1117] border border-gray-200 dark:border-gray-700 rounded-full py-2 px-6 text-xs font-bold text-blue-600 shadow-lg focus:ring-4 ring-blue-500/10 outline-none cursor-pointer transition-all">
                    ...
                    {item.type === 'COPY' && (
                      <div className="flex items-center gap-2 bg-white dark:bg-[#0D1117] px-3 py-1.5 rounded-full border border-gray-100 dark:border-gray-800 shadow-inner">
                        ...
                        <input type="text" placeholder="--from" value={item.from} onChange={(e) => updateArrayField(copyAddItems, setCopyAddItems, i, "from", e.target.value)}
                          className={`w-[80px] bg-transparent outline-none text-xs font-bold text-blue-500`} />
                      </div>
                    )}
                  </div>
                  <div className="flex-1 w-full flex items-center gap-4">
                    <div className="flex-1 relative group">
                      <input type="text" placeholder="Source pattern..." value={item.src} onChange={(e) => updateArrayField(copyAddItems, setCopyAddItems, i, "src", e.target.value)}
                        className={`${inpSm} !h-[44px] !py-3 !rounded-2xl !bg-white dark:!bg-[#0D1117] border-blue-200/50 font-mono shadow-md focus:ring-4 ring-blue-500/5`} />
                      <p className="absolute -top-2.5 left-4 px-2 py-0.5 bg-white dark:bg-[#0D1117] text-xs font-black text-blue-600 dark:text-blue-400 tracking-wider shadow-sm border border-blue-100 dark:border-blue-800 rounded-lg z-10">{t.dockerfile.src}</p>
                    </div>
                    ...
                    <div className="flex-1 relative group">
                      <input type="text" placeholder="Container dest..." value={item.dest} onChange={(e) => updateArrayField(copyAddItems, setCopyAddItems, i, "dest", e.target.value)}
                        className={`${inpSm} !h-[44px] !py-3 !rounded-2xl !bg-white dark:!bg-[#0D1117] border-blue-200/50 font-mono shadow-md focus:ring-4 ring-blue-500/5`} />
                      <p className="absolute -top-2.5 left-4 px-2 py-0.5 bg-white dark:bg-[#0D1117] text-xs font-black text-blue-600 dark:text-blue-400 tracking-wider shadow-sm border border-blue-100 dark:border-blue-800 rounded-lg z-10">{t.dockerfile.dest}</p>
                    </div>
                  </div>
              ...
              {copyAddItems.length === 0 && <div className="py-10 border-2 border-dashed border-blue-200 dark:border-gray-800 rounded-[2rem] flex flex-col items-center justify-center opacity-60 italic text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal">{t.dockerfile.noResources}</div>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-10 border-t-2 border-gray-100 dark:border-gray-800">
            <div className="p-8 bg-indigo-50/20 dark:bg-indigo-900/10 rounded-[3rem] border border-indigo-100 dark:border-indigo-900/30 group hover:shadow-xl transition-all duration-500 relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <p className="text-sm font-bold text-indigo-600 dark:text-indigo-400 tracking-normal">{t.dockerfile.expose}</p>
              </div>
              <div className="relative group">
                ...
                <div className="flex justify-between items-center mt-3">
                  <p className="text-xs text-gray-500 dark:text-gray-400 font-bold italic">{t.dockerfile.exposeDesc || 'Declare internal container ports'}</p>
                  <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 tracking-normal bg-indigo-50/50 dark:bg-indigo-900/40 px-2.5 py-1 rounded-xl shadow-sm">{t.dockerfile.exposeHint || 'Space separated'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-8 bg-emerald-50/20 dark:bg-emerald-900/10 rounded-[3rem] border border-emerald-100 dark:border-emerald-900/30 group hover:shadow-xl transition-all duration-500 relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <p className="text-sm font-bold text-emerald-600 dark:text-emerald-400 tracking-normal transition-colors">{t.dockerfile.healthcheck}</p>
              </div>
              <div className="space-y-6">
                <div className="space-y-1.5">
                  <p className="text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.healthcheckCmd}</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <p className="text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.interval}</p>
                    <input type="text" placeholder="30s" className={`${inpSm} !py-2.5 !bg-white dark:!bg-[#0D1117] text-center font-bold shadow-md`} />
                  </div>
                  <div className="space-y-1.5">
                    <p className="text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.timeout}</p>
                    <input type="text" placeholder="5s" className={`${inpSm} !py-2.5 !bg-white dark:!bg-[#0D1117] text-center font-bold shadow-md`} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="md:col-span-2 p-8 bg-gray-50/50 dark:bg-[#161B22]/50 rounded-[3rem] border border-gray-200 dark:border-gray-800 shadow-inner group">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.volume}</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {volumes.length === 0 && <div className="md:col-span-2 py-8 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-[2rem] flex flex-col items-center justify-center opacity-40 italic text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal">{t.dockerfile.noVolumesDefined}</div>}
              </div>
            </div>
          </div>
        </div>
      ...
      {/* 4. Templates */}
      <h2 className="text-sm font-bold text-gray-400 dark:text-gray-500 mb-10 mt-16 flex items-center justify-center gap-8 tracking-normal transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {[
          ...
          <button key={tpl.title} onClick={tpl.action}
            ...
            <h3 className="text-sm font-bold text-gray-900 dark:text-gray-100 mb-3 truncate pr-6 tracking-tight transition-colors">{tpl.title}</h3>
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400 leading-relaxed mb-2.5 ml-1 transition-colors">{tpl.desc}</p>
            <div className="mt-8 flex items-center gap-2 text-sm font-bold text-gray-600 dark:text-gray-300 tracking-normal opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
