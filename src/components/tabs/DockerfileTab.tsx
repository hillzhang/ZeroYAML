...
const inpSm = "w-full bg-white dark:bg-[#0D1117] border border-gray-200 dark:border-gray-800 rounded-lg py-1.5 px-3 text-[13px] font-bold h-[36px] focus:outline-none focus:border-blue-400 transition-all text-gray-800 dark:text-gray-200";
    ...
    <div className={`p-5 rounded-3xl border border-l-8 ${config} transition-all duration-300 shadow-sm hover:shadow-md animate-in fade-in slide-in-from-bottom-2`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <p className={`text-[12px] font-bold tracking-normal ${textColor}`}>{title}</p>
        </div>
        <span className="text-[11px] bg-white dark:bg-black/40 text-gray-700 dark:text-gray-200 px-3 py-1 rounded-full font-bold border border-gray-100 dark:border-gray-800 shadow-inner transition-colors">{items.length} Items</span>
      </div>
...
export function DockerfileTab() {
  ...
  return (
    <div className="animate-in fade-in duration-700 flex flex-col pb-20 pt-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[12px] font-bold text-blue-600 dark:text-blue-400 tracking-normal leading-none">{t.dockerfile.startConfig}</p>
                  <p className="text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.executionLogic}</p>
                </div>
              </div>
            ...
            {(entrypoint || startCmd) && (
              <div className="px-5 py-2 bg-white/80 dark:bg-black/40 rounded-2xl border border-blue-100 dark:border-blue-900/30 shadow-sm backdrop-blur-md transition-transform hover:scale-[1.02]">
                <p className="text-[11px] font-mono text-blue-600 dark:text-blue-400 truncate max-w-[300px]">
                  <span className="opacity-50 text-[10px] font-sans mr-2 font-bold">Preview:</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative items-start">
            ...
            <div className="space-y-3 relative group">
              <div className="flex items-center justify-between px-1">
                ...
                <span className="text-[11px] font-bold text-blue-700 bg-blue-100 dark:text-blue-300 dark:bg-blue-800/60 px-3 py-1 rounded-full shadow-sm">FIXED</span>
              </div>
            ...
            <div className="space-y-3 relative group">
              <div className="flex items-center justify-between px-1">
                ...
                <span className="text-[11px] font-bold text-emerald-700 bg-emerald-100 dark:text-emerald-300 dark:bg-emerald-800/60 px-3 py-1 rounded-full shadow-sm">ARGS</span>
              </div>
          ...
          <div className="p-8 bg-gray-50/50 dark:bg-[#161B22]/50 border border-gray-200 dark:border-gray-800 rounded-[2.5rem] shadow-inner relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[12px] font-bold text-gray-800 dark:text-gray-200 tracking-normal">{t.dockerfile.run}</p>
                  <p className="text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.layerScripts}</p>
                </div>
              </div>
            ...
            <div className="space-y-4 relative z-10">
              ...
              {runCmds.length === 0 && <div className="py-10 border-2 border-dashed border-orange-200 dark:border-orange-800 rounded-[2rem] flex flex-col items-center justify-center opacity-40 italic text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal">{t.dockerfile.noSteps}</div>}
            </div>
          </div>
          ...
          <div className="pt-10 border-t-2 border-gray-100 dark:border-gray-800">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[12px] font-bold text-gray-800 dark:text-gray-200 tracking-normal">{t.dockerfile.addCopy}</p>
                  <p className="text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.srcDestMapping}</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                <div key={i} className="flex flex-col lg:flex-row gap-4 items-center bg-gray-50/50 dark:bg-[#161B22]/50 p-4 rounded-3xl border border-gray-100 dark:border-gray-800 hover:border-blue-400/50 transition-all group/file shadow-sm">
                  <div className="flex items-center gap-3">
                    <select value={item.type} onChange={(e) => updateArrayField(copyAddItems, setCopyAddItems, i, "type", e.target.value)}
                      className="bg-white dark:bg-[#0D1117] border border-gray-200 dark:border-gray-700 rounded-full py-2 px-6 text-[11px] font-bold text-blue-600 shadow-lg focus:ring-4 ring-blue-500/10 outline-none cursor-pointer transition-all">
                    ...
                    {item.type === 'COPY' && (
                      <div className="flex items-center gap-2 bg-white dark:bg-[#0D1117] px-3 py-1.5 rounded-full border border-gray-100 dark:border-gray-800 shadow-inner">
                        <span className="text-[10px] text-gray-400 font-bold">FROM</span>
                        <input type="text" placeholder="--from" value={item.from} onChange={(e) => updateArrayField(copyAddItems, setCopyAddItems, i, "from", e.target.value)}
                          className={`w-[80px] bg-transparent outline-none text-[11px] font-bold text-blue-500`} />
                      </div>
                    )}
                  </div>
                  <div className="flex-1 w-full flex items-center gap-4">
                    <div className="flex-1 relative group">
                      ...
                      <p className="absolute -top-2.5 left-4 px-2 py-0.5 bg-white dark:bg-[#0D1117] text-[10px] font-black text-blue-600 dark:text-blue-400 tracking-wider shadow-sm border border-blue-100 dark:border-blue-800 rounded-lg z-10">{t.dockerfile.src}</p>
                    </div>
                    ...
                    <div className="flex-1 relative group">
                      <input type="text" placeholder="Container dest..." value={item.dest} onChange={(e) => updateArrayField(copyAddItems, setCopyAddItems, i, "dest", e.target.value)}
                        className={`${inpSm} !h-[44px] !py-3 !rounded-2xl !bg-white dark:!bg-[#0D1117] border-blue-200/50 font-mono shadow-md focus:ring-4 ring-blue-500/5`} />
                      <p className="absolute -top-2.5 left-4 px-2 py-0.5 bg-white dark:bg-[#0D1117] text-[10px] font-black text-blue-600 dark:text-blue-400 tracking-wider shadow-sm border border-blue-100 dark:border-blue-800 rounded-lg z-10">{t.dockerfile.dest}</p>
                    </div>
                  </div>
              ...
              {copyAddItems.length === 0 && <div className="py-10 border-2 border-dashed border-blue-200 dark:border-gray-800 rounded-[2rem] flex flex-col items-center justify-center opacity-60 italic text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal">{t.dockerfile.noResources}</div>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-10 border-t-2 border-gray-100 dark:border-gray-800">
            <div className="p-8 bg-indigo-50/20 dark:bg-indigo-900/10 rounded-[3rem] border border-indigo-100 dark:border-indigo-900/30 group hover:shadow-xl transition-all duration-500 relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <p className="text-[12px] font-bold text-indigo-600 dark:text-indigo-400 tracking-normal">{t.dockerfile.expose}</p>
              </div>
              <div className="relative group">
                ...
                <div className="flex justify-between items-center mt-3">
                  ...
                  <p className="text-[11px] font-bold text-indigo-600 dark:text-indigo-400 tracking-normal bg-indigo-50/50 dark:bg-indigo-900/40 px-2.5 py-1 rounded-xl shadow-sm">{t.dockerfile.exposeHint || 'Space separated'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-8 bg-emerald-50/20 dark:bg-emerald-900/10 rounded-[3rem] border border-emerald-100 dark:border-emerald-900/30 group hover:shadow-xl transition-all duration-500 relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <p className="text-[12px] font-bold text-emerald-600 dark:text-emerald-400 tracking-normal transition-colors">{t.dockerfile.healthcheck}</p>
              </div>
              <div className="space-y-6">
                <div className="space-y-1.5">
                  <p className="text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.healthcheckCmd}</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <p className="text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.interval}</p>
                    <input type="text" placeholder="30s" className={`${inpSm} !py-2.5 !bg-white dark:!bg-[#0D1117] text-center font-bold shadow-md`} />
                  </div>
                  <div className="space-y-1.5">
                    <p className="text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.timeout}</p>
            ...
            <div className="md:col-span-2 p-8 bg-gray-50/50 dark:bg-[#161B22]/50 rounded-[3rem] border border-gray-200 dark:border-gray-800 shadow-inner group">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal mb-2.5 ml-1">{t.dockerfile.volume}</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {volumes.length === 0 && <div className="md:col-span-2 py-8 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-[2rem] flex flex-col items-center justify-center opacity-40 italic text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal">{t.dockerfile.noVolumesDefined}</div>}
              </div>
            </div>
          </div>
        </div>
      ...
      {/* 4. Templates */}
      <h2 className="text-[13px] font-bold text-gray-400 dark:text-gray-500 mb-10 mt-16 flex items-center justify-center gap-8 tracking-normal transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {[
          ...
          <button key={tpl.title} onClick={tpl.action}
            ...
            <div className="mt-8 flex items-center gap-2 text-[12px] font-bold text-gray-600 dark:text-gray-300 tracking-normal opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
