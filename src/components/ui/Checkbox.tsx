...
export function Checkbox({ checked, onChange, label, className = "", theme = 'blue' }: CheckboxProps) {
  ...
  return (
    ...
    >
      ...
      {label && (
        <span className={`text-xs font-bold tracking-tight transition-colors duration-300 ${checked ? themes[theme].split(' ').pop() : 'text-gray-500 dark:text-gray-400 group-hover:text-blue-500'}`}>
          {label}
        </span>
      )}
    </div>
